...
const usersData = async () => {
  ...
  try {
    ...
    const response = await fetch("https://peluqueria-invasion.vercel.app/users");
...
const submitEmployee = (form, modal, modalFooter) => {
  ...
  form.addEventListener('submit', (e) => {
      ...
      let url = 'https://peluqueria-invasion.vercel.app/register';
      let method = 'POST';

      // Preguntamos si el modo es update para hacer una correcta request.
      if (mode === 'update') {
          url = `https://peluqueria-invasion.vercel.app/users/${id}`;
          method = 'PUT';
      };
...
const updateEmployee = (btnsPut, modal) => {
  ...
  btnsPut.forEach(btn => {
      ...
      btn.addEventListener('click', async (e) => {
          ...
          const response = await fetch(`https://peluqueria-invasion.vercel.app/users/${key}`);
          const data = await response.json();
