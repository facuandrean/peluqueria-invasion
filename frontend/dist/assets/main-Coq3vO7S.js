/* empty css                  */(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const s of n.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function l(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function o(a){if(a.ep)return;a.ep=!0;const n=l(a);fetch(a.href,n)}})();var H={code:"es",week:{dow:1,doy:4},buttonText:{prev:"Ant",next:"Sig",today:"Hoy",year:"Año",month:"Mes",week:"Semana",day:"Día",list:"Agenda"},buttonHints:{prev:"$0 antes",next:"$0 siguiente",today(e){return e==="Día"?"Hoy":(e==="Semana"?"Esta":"Este")+" "+e.toLocaleLowerCase()}},viewHint(e){return"Vista "+(e==="Semana"?"de la":"del")+" "+e.toLocaleLowerCase()},weekText:"Sm",weekTextLong:"Semana",allDayText:"Todo el día",moreLinkText:"más",moreLinkHint(e){return`Mostrar ${e} eventos más`},noEventsText:"No hay eventos para mostrar",navLinkHint:"Ir al $0",closeHint:"Cerrar",timeHint:"La hora",eventHint:"Evento"};function T(e){const[t,l]=e.split("T"),[,o,a]=t.split("-"),[n,s]=l.split(":"),r=`${o}-${a}`,i=`${n}:${s}`;return{dayWithoutYear:r,timeWithoutSeconds:i,completeDate:e}}const y=`
...
`;function D(e){const t=document,l=new bootstrap.Modal(t.getElementById("dateClickModalTurnContent"));t.getElementById("infoName"),t.getElementById("infoTel"),t.getElementById("infoDay"),t.getElementById("infoStartTime"),t.getElementById("infoEndTime");const o=t.getElementById("spanName"),a=t.getElementById("spanTel"),n=t.getElementById("spanDay"),s=t.getElementById("spanStartTime"),r=t.getElementById("spanEndTime"),{dayWithoutYear:i,timeWithoutSeconds:m}=T(e.event.startStr),{timeWithoutSeconds:d}=T(e.event.endStr),p=e.event._def.title,u=e.event._def.extendedProps.telefono,c=i,b=m,g=d;o.innerHTML="",a.innerHTML="",n.innerHTML="",s.innerHTML="",r.innerHTML="",o.innerHTML+=`${p}`,a.innerHTML+=`${u}`,n.innerHTML+=`${c}`,s.innerHTML+=`${b}`,r.innerHTML+=`${g}`,l.show();const $=document.getElementById("deleteTurn"),f=document.getElementById("contactWsp");$.addEventListener("click",async E=>{E.preventDefault(),alert("sad")}),f.addEventListener("click",async E=>{E.preventDefault();const A=`Hola ${p} espero que te encuentres muy bien!, tenes un turno agendado para el dia ${c}, a las ${b}`,w=`https://api.whatsapp.com/send?phone=${u}&text=${encodeURIComponent(A)}`;f.href=w,window.open(w,"_blank")})}function N(e,t,l){const o=document,a=new bootstrap.Modal(o.getElementById("dateClickModal")),n=o.getElementById("eventDate"),s=o.getElementById("event-datetime"),{dayWithoutYear:r,timeWithoutSeconds:i,completeDate:m}=T(e.dateStr),d=o.getElementById("eventForm");d.inputName.value="",d.inputNumber.value="",n.value=r,s.value=i,a.show(),document.querySelector(".btnCancel").addEventListener("click",u=>{u.preventDefault(),bootstrap.Modal.getInstance(a._element).hide()}),x(d,m,l,a)}async function x(e,t,l,o){const a=document.querySelector(".modal-footer"),n=document.createElement("span");n.innerHTML="Error al crear el turno.",n.style.textAlign="center",n.style.width="100%",n.style.marginTop="1rem",n.style.marginBottom="0rem",n.style.paddingBottom="0rem",document.getElementById("input-name").addEventListener("input",function(s){const r=s.target;/^[a-zA-Z\s]{1,25}$/.test(r.value)?r.setCustomValidity(""):r.setCustomValidity("solo letras y espacios hasta 25 caracteres.")}),document.getElementById("input-number").addEventListener("input",function(s){const r=s.target;/^\+?\d{1,15}$/.test(r.value)?r.setCustomValidity(""):r.setCustomValidity("Solo numeros hasta 15 digitos.")}),e.addEventListener("submit",async s=>{s.preventDefault();const r=document.getElementById("input-name"),i=document.getElementById("input-number"),m=e.inputName.value.trim(),d=e.inputNumber.value.trim(),p=l.user.Id,u=t;r.checkValidity()&&i.checkValidity()?console.log("Formulario válido. Enviando datos..."):(r.reportValidity(),i.reportValidity());const c={Nombre:m,Telefono:d,Date:u,NroUsuario:p},b="http://localhost:3001/turns",g={method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(c)};(await fetch(b,g)).ok?(n.innerHTML="Turno creado correctamente",n.style.color="green",setTimeout(()=>{bootstrap.Modal.getInstance(o._element).hide(),window.location.reload()},1500)):n.style.color="red",a.appendChild(n)})}const B=document;let C=document.body;const P=async e=>await(await fetch(`http://localhost:3001/turns/${e.user.Id}`)).json(),q=e=>{const[t,l]=e.split("T"),[o,a]=l.split(":");let n=o,s="";a==="00"?s="30":a==="30"&&(n=parseInt(n)+1,s="00"),n===9&&(n="09");const r=`${n}:${s}`;return`${t}T${r}`};async function h(e,t){const o=(await P(t)).map(s=>{const r=q(s.turns.Date);return{id:s.turns.Id,title:s.turns.Nombre,start:s.turns.Date,end:r,extendedProps:{telefono:s.turns.Telefono}}});let a=B.getElementById("calendar"),n=new FullCalendar.Calendar(a,{initialView:"timeGridWeek",timeZone:"America/Argentina/Cordoba",slotLabelFormat:{hour:"numeric",minute:"2-digit",omitZeroMinute:!1},slotLabelInterval:"00:30:00",slotDuration:"00:30:00",slotMinTime:"08:00:00",editable:!0,dayMaxEventRows:!0,views:{timeGrid:{dayMaxEventRows:6},dayGrid:{dayMaxEventRows:3}},allDaySlot:!1,headerToolbar:{left:"dayGridMonth,timeGridWeek,timeGridDay,myCustomButton",center:"title",right:"prev,next"},events:o,eventClick:function(s){C.insertAdjacentHTML("beforeend",I),D(s),document.querySelector(".modal").addEventListener("hidden.bs.modal",function(){this.remove()})},customButtons:{myCustomButton:{text:"boton personalizado",click:function(){alert("funcionalidad del boton personalizado")}}},dateClick:function(s){L(),C.insertAdjacentHTML("beforeend",e),N(s,n,t),document.querySelector(".modal").addEventListener("hidden.bs.modal",function(){this.remove()})},selectable:!1,locale:H});n.render()}const v='<div class="contenedorCalendario" id="calendar"></div>',R=e=>`
        <aside class="sidebar">
    ...
    `,O=()=>{const e=document.querySelector(".btnHamburger button");e.removeEventListener("click",M),e.addEventListener("click",M)},M=e=>{e.currentTarget.classList.toggle("is-active")},k=async()=>{try{(await fetch("http://localhost:3001/logout",{method:"POST",credentials:"include"})).ok&&(history.replaceState(null,"","/login.html"),history.pushState(null,"","/login.html"),window.location.href="/login.html")}catch(e){console.error(e)}};let j='<div class="manageEmployeesContainer"></div>';const V=`
...
`,F=e=>{let t="";return e.forEach((l,o)=>{o>0&&(t+=`
        <tr key=${l.Id}>
          ...
          <td>
            <button class="table-btns modify">
              <i class="bi bi-pencil-fill" key=${l.Id}></i>
            </button>
      ...
      `)}),t},G=async()=>{try{const e=await fetch("http://localhost:3001/users");if(!e.ok)alert("Hubo algun error en obtener los usuarios.");else{const t=await e.json();return t.length>1?`
        ...
        `:'<p class="empty">No hay empleados registrados.</p>'}}catch(e){console.log(e)}},U=e=>{e.addEventListener("click",()=>{document.querySelector("#postEmployeeLabel").textContent="Registrar empleado",document.querySelector(".btnPost").textContent="Registrar";const t=document.querySelector("#formPOSTEmployee");t.setAttribute("data-mode","create"),t.removeAttribute("data-id")})},_=(e,t,l)=>{const o=document.createElement("span");o.innerHTML="Error al crear el usuario.",o.style.textAlign="center",o.style.width="100%",o.style.marginTop="1rem",o.style.marginBottom="0rem",o.style.paddingBottom="0rem",e.addEventListener("submit",a=>{a.preventDefault();const n=e.getAttribute("data-mode"),s=e.getAttribute("data-id"),r=e.Nombre.value,i=e.Contrasena.value,m=e.Rol.value,d={Nombre:r,Contrasena:i,Rol:m};let p="http://localhost:3001/register",u="POST";n==="update"&&(p=`http://localhost:3001/users/${s}`,u="PUT"),fetch(p,{method:u,headers:{"Content-Type":"application/json"},body:JSON.stringify(d)}).then(c=>c.json()).then(c=>{c.error!==void 0?(o.innerHTML=`${c.error}`||"Usuario o contraseña inválidos.",o.style.color="red"):(o.innerHTML=n==="create"?"¡Usuario creado correctamente!":"¡Usuario actualizado correctamente!",o.style.color="green",setTimeout(()=>{bootstrap.Modal.getInstance(t._element).hide(),window.location.reload()},1500)),l.appendChild(o)}).catch(c=>{console.log("Error del servidor:",c)})})},z=(e,t,l)=>{e.addEventListener("click",o=>{o.preventDefault(),t.Contrasena.value="",t.Nombre.value="",bootstrap.Modal.getInstance(l._element).hide()})},Z=(e,t)=>{e.forEach(l=>{l.addEventListener("click",async o=>{const a=o.currentTarget.getAttribute("key"),s=await(await fetch(`http://localhost:3001/users/${a}`)).json();document.querySelector("#postEmployeeLabel").textContent="Actualizar empleado",document.querySelector(".btnPost").textContent="Actualizar";const r=document.querySelector("#formPOSTEmployee");r.setAttribute("data-mode","update"),r.setAttribute("data-id",a),r.Contrasena.value="",r.Nombre.value=s.Nombre,r.Contrasena.placeholder="Contraseña",r.Rol.value=s.Rol,t.show()})})},Y=e=>{e.forEach(t=>{t.addEventListener("click",async l=>{const o=l.currentTarget.closest("tr").getAttribute("key"),n=await(await fetch(`http://localhost:3001/users/${o}`)).json();if(console.log(n),confirm(`¿Estás seguro que quieres eliminar al empleado ${n.Nombre}?`)){const r=await fetch(`http://localhost:3001/users/${o}`,{method:"DELETE"});console.log(r),r.ok?window.location.reload():alert("Error al eliminar el usuario.")}})})},J=async e=>{const t=e.user.Nombre,l=window.location.hash;switch(app.innerHTML="",app.innerHTML+=R(t),l){case"#admin-calendar":app.innerHTML+=v,h(y,e);break;case"#cash-register":break;case"#share-calendar":break;case"#manage-employees":app.innerHTML+=j;let a=document.querySelector(".manageEmployeesContainer");a.insertAdjacentHTML("beforeend",V);const n=await G();n&&a.insertAdjacentHTML("beforeend",n),a.insertAdjacentHTML("beforeend",W);const s=document.querySelector(".postEmployee-btn");s.setAttribute("data-bs-toggle","modal"),s.setAttribute("data-bs-target","#postEmployee");const r=new bootstrap.Modal(document.getElementById("postEmployee"));U(s);const i=document.querySelector("#formPOSTEmployee"),m=document.querySelector(".modal-footer");_(i,r,m);const d=document.querySelector(".btnCancel");z(d,i,r);const p=document.querySelectorAll(".modify i");Z(p,r);const u=document.querySelectorAll(".delete i");Y(u);break;case"#generate-table":break;default:app.innerHTML+=v,h(y,e);break}O(),document.querySelector("#logout").addEventListener("click",k)},K=e=>`
        <aside class="sidebar">
            <div class="sidebar-nav">
                <div class="btnHamburger">
                    <button class="hamburger hamburger--collapse" type="button">
